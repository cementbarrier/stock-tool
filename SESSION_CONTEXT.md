# BiliDigest 项目上下文（2026-07-23）

BiliDigest——B 站视频字幕提取与 AI 摘要工具。基于 bili2text 转写引擎，当前摘要模板固定为财经市场方向。
...
- **桌面快捷方式**：`BiliDigest` → `E:\stock-tool\dist\gui.exe`
...
| # | 问题 | 根因 | 修复 |
|---|------|------|------|
| 26 | 打包后 scripts 子模块缺失 | gui.spec 未包含 scripts 目录及 step 模块 | `hiddenimports` 补充所有 step 模块，`datas` 包含 scripts 目录 |
| 27 | PyInstaller 未收集深层子模块 | 默认只收集顶层 import，scripts/step*.py 的间接依赖遗漏 | gui.spec 添加 `collect_submodules` 递归收集 |
| 28 | pathex 未包含项目根目录 | gui.spec 未设置 `pathex`，PyInstaller 可能找不到源码 | 添加 `pathex=['.']` 确保从项目根目录搜索 |
